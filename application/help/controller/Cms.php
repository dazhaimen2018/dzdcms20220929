...
use app\help\model\Cms as Cms_Model;
...
class Cms extends Adminbase
{
    ...
    public function remove()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($ids) {
                ...
                $modelid = getHelp($tocatid, 'modelid');
                if (!$modelid) {
                    $this->error('该模型不存在！');
                }
    ...
    public function public_helps()
    {
        ...
        if ($isAdministrator !== true) {
            ...
            $priv_result = Db::name('HelpPriv')->where(['roleid' => $role_id, 'action' => 'init'])->select();
        ...
        foreach ($helps as $rs) {
            ...
            $data['target'] = 'right';
            $data['url']    = url('help/cms/classlist', array('catid' => $rs['id']));
            //单页
            if ($rs['type'] == 1) {
                $data['target'] = 'right';
                $data['url']    = url('help/cms/add', array('catid' => $rs['id']));
            }
            $json[] = $data;
        }
    ...
    protected function check_priv($action)
    {
        if (User::instance()->isAdministrator() !== true) {
            $catid      = $this->request->param('catid/d', 0);
            $action     = getHelp($catid, 'type') == 1 ? 'init' : $action;
            $priv_datas = Db::name('HelpPriv')->where(['catid' => $catid, 'is_admin' => 1, 'roleid' => User::instance()->roleid, 'action' => $action])->find();
            if (empty($priv_datas)) {
