...
class Cms extends Modelbase
{
    ...
    public function addModelData($data, $dataExt = [])
    {
        ...
        //更新栏目统计数据
        $this->updateHelpItems($catid, 'add', 1);
        //推送到熊掌号和百度站长
    ...
    public function deleteModelData($modeId, $id, $no_delete = false)
    {
        ...
        if ($no_delete) {
            ...
            //更新栏目统计
            $this->updateHelpItems($data['catid'], 'delete');
        }
        //标签
    ...
    public function getList($modeId, $where, $moreifo, $field = '*', $order = '', $limit, $page = null, $simple = false, $config = [])
    {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $key => $vo) {
                ...
                $cat          = $url_mode == 1 ? $vo['catid'] : (isset($Help[$vo['catid']]) ? $Help[$vo['catid']]['catdir'] : getHelp($vo['catid'], 'catdir'));
    ...
    public function getContent($modeId, $where, $moreifo = false, $field = '*', $order = '', $cache = false)
    {
        ...
        if (!empty($dataInfo)) {
            ...
            $cat             = $url_mode == 1 ? $dataInfo['catid'] : (isset($Help[$dataInfo['catid']]) ? $Help[$dataInfo['catid']]['catdir'] : getHelp($dataInfo['catid'], 'catdir'));
    ...
    private function updateHelpItems($catid, $action = 'add', $cache = 0)
    {
        if ($action == 'add') {
            Db::name('Help')->where('id', $catid)->setInc('items');
        } else {
            Db::name('Help')->where('id', $catid)->where('items', '>', 0)->setDec('items');
        }
    }
