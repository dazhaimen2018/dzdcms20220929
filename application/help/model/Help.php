...
class Help extends Model
{
    ...
    //新增栏目
    public function addHelp($data, $fields)
    {
        if (empty($data)) {
    ...
    //编辑栏目
    public function editHelp($data, $fields)
    {
        if (empty($data)) {
        ...
        if ($data['template_child']) {
            ...
            if (!empty($idstr)) {
                ...
                foreach ($arr as $key => $val) {
                    ...
                    $setting['help_template'] = $data['setting']['help_template'];
    ...
    {
        if (empty($this->helps)) {
            $this->helps = cache('Help');
        }
        if ($n > 10 || !is_array($this->helps) || !isset($this->helps[$catid])) {
            return false;
        }
        //获取当前栏目的上级栏目ID
        $parentid = $this->helps[$catid]['parentid'];
        //所有父ID
        $arrparentid = $arrparentid ? $parentid . ',' . $arrparentid : $parentid;
        if ($parentid) {
            $arrparentid = $this->get_arrparentid($parentid, $arrparentid, ++$n);
        } else {
            $this->helps[$catid]['arrparentid'] = $arrparentid;
        }
        //$parentid = $this->helps[$catid]['parentid'];
        return $arrparentid;
    }
